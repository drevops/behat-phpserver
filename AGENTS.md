...
Each job also picks a Guzzle major from its dependency level. The `normal` jobs add `--with="guzzlehttp/guzzle:^8"` and the `lowest` jobs add `--with="guzzlehttp/guzzle:^7"`, so Guzzle 8 runs at its newest release and Guzzle 7 at the floor of the `^7.15.3 || ^8.2.0` range in `composer.json`. That range resolves to the same versions without the pins. The pins make the install fail when another dependency stops allowing either major, so the matrix can't lose a major without a failing job.
